...
def simple_auction():
    ...
    print("Proof of Impact :", seller.getAddress())
    print("Auction Account:", creator.getAddress())
    print("Client (bidder account)", bidder.getAddress(), "\n")

    print("Proof of Impact is generating an example NFT...")
    nftAmount = 1
    nftID = createDummyAsset(client, nftAmount, seller)
    print("The NFT ID is", nftID)
    print("Proof of Impact's balances:", getBalances(client, seller.getAddress()), "\n")
    ...
    print("Auction Account is creating an auction that lasts 30 seconds to auction off the NFT...")
    ...
    print("Proof of Impact is setting up and funding NFT auction...")
    setupAuctionApp(
        client=client,
        appID=appID,
        funder=creator,
    ...
    print("Proof of Impact's balances:", sellerBalancesBefore)
    ...
    print("Client wants to bid on NFT, her balances:", bidderBalancesBefore)
    print("Client is placing bid for", bidAmount, "microAlgos")

    placeBid(client=client, appID=appID, bidder=bidder, bidAmount=bidAmount)

    print("Client is opting into NFT with ID", nftID)
    ...
    print("Proof of Impact is closing out the auction\n")
    ...
    print("Proof of Impact's balances after auction: ", actualSellerBalances, " Algos")
    actualBidderBalances = getBalances(client, bidder.getAddress())
    print("Client's balances after auction: ", actualBidderBalances, " Algos")
